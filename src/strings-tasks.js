...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replace(/\W|_/g, '').split('').join('');
  const str2 = str
    .toLowerCase()
    .replace(/\W|_/g, '')
    .split('')
...
function encodeToRot13(str) {
  ...
  let result = '';
  const keys = Object.keys(rot13);

  for (let i = 0; i < str.length; i += 1) {
    let currentLetter;
    if (!keys.includes(str[i].toLowerCase())) {
      currentLetter = str[i];
    } else if (str[i] === str[i].toUpperCase()) {
      currentLetter = rot13[str[i].toLowerCase()].toUpperCase();
    } else {
      currentLetter = rot13[str[i]];
    }
